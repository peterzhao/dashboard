...
describe Ju::Config do
  ...
  it 'should get board config' do
    ...
    expect(config['widgets'][0]['url']).to eq('http://abc.com/')
    ...
    expect(config['widgets'][1]['url']).to eq('http://cde.com/')
  ...
  context 'saving widget' do
    it 'should add new widget' do
      ...
      Ju::Config.save_widget('temp', 'travis', data, nil)
      ...
      expect(Ju::Config.get_widget_config('temp', 'widget2')['type']).to eq('curl')
    end
