...
describe Ju::GocdBuild::Transformer do
  ...
  context 'state' do
    ...
    it 'should transform data' do
      pipeline_data = Ju::GocdBuild::Transformer.transform(data, "build")
      expect(pipeline_data['id']).to eq('3')
      expect(pipeline_data['stage']['name']).to eq('build')
      expect(pipeline_data['stage']['state']).to eq('Building')
      expect(pipeline_data['stage']['result']).to eq('Failed')
      expect(pipeline_data['stage']['scheduled_time']).to eq('1 minute ago')
    end
  end
